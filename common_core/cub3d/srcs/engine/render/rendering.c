...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2020/12/18 16:57:09 by azod              #+#    #+#             */
/*   Updated: 2021/01/05 15:24:18 by azod             ###   ########lyon.fr   */
/*                                                                            */
/* ************************************************************************** */
...
int hit(t_display *d, int **worldmap, int or)
{
	...
	if (d->side == 0)
		d->perpwall_dist = (d->map_x - d->pos_x + (1 - d->step_x) / 2) / d->raydir_x;
	else
		d->perpwall_dist = (d->map_y - d->pos_y + (1 - d->step_y) / 2) / d->raydir_y;
	return (or);
}

void define_startend(t_display *d, t_params *p)
{
	d->draw_start = (-d->lineheight / 2 + p->win_y / 2) + d->pitch +
					(d->pos_z / d->perpwall_dist);
	if (d->draw_start < 0)
		d->draw_start = 0;
	d->draw_end = (d->lineheight / 2 + p->win_y / 2) + d->pitch +
				  (d->pos_z / d->perpwall_dist);
...
void get_texture(t_display *d, t_params *p, t_all *all, int or)
{
	...
	d->tex_pos = (d->draw_start - d->pitch - (d->pos_z / d->perpwall_dist) -
				  p->win_y / 2 + d->lineheight / 2) *
				 d->step;
}
