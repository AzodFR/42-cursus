...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/01/11 11:55:34 by azod              #+#    #+#             */
/*   Updated: 2021/02/16 16:38:11 by azod             ###   ########lyon.fr   */
/*                                                                            */
/* ************************************************************************** */
...
static int treat_export(char *arg, t_env *env)
{
	...
	else
	{
		if (!env_find(env, get_name(arg)))
			...
			env_edit_value(env_find(env, get_name(arg)), is_cat(arg) ? ft_strjoin(env_find(env, get_name(arg))->value,
																				  get_value(arg))
																	 : get_value(arg));
