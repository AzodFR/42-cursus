...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/02/15 14:10:05 by azod              #+#    #+#             */
/*   Updated: 2021/03/02 10:56:30 by azod             ###   ########lyon.fr   */
/*                                                                            */
/* ************************************************************************** */
...
void prefix(t_params *p)
{
	...
	printf("%.0f [\033[93mPhilosophers\033[0m] ", p ? (double)(actual.tv_usec - p->start.tv_usec) / 1000 + (double)(actual.tv_sec - p->start.tv_sec) * 1000 : 0);
}
...
void print_action(t_philos *p, char *s)
{
	...
	gettimeofday(&actual, NULL);
	time = (double)(actual.tv_usec - p->p->start.tv_usec) / 1000 + (double)(actual.tv_sec - p->p->start.tv_sec) * 1000;
	printf("%.0f \033[9%dm%d %s\033[0m\n", time > 2 ? time - 2 : time, p->id % 6 + 1, p->id + 1, s);
